...
def shortest_path(We, graph, start, end, token):
    n = len(We)
    ...
    if adjacent[start, end] == 1:
        ...
        y = np.array(
            [Binary(f"y{i}") if (i != end) and (i != start) else 1 for i in range(n)]
        )
        ...
        data = pd.DataFrame(
            [sampleset.record[i][0] for i in range(len(sampleset.record))],
            columns=sampleset.variables,
        )

        # Take out samples that fulfill the constraints and are connected graph
        feasible_index = np.where(sampleset.record.is_feasible)[0]
        shortest_dist = np.min(sampleset.record[feasible_index].energy)
        optimal_index = np.where(
            sampleset.record[feasible_index].energy
            == np.min(sampleset.record[feasible_index].energy)
        )[0]
...
def ss_shortest_path(We, graph, start, token):
    n = len(We)
    adjacent = adjacent_matrix(graph, n)
    ...
    y = np.array(
        [
            [Binary(f"y{i}{j}") if (j != i) and (j != start) else 1 for j in range(n)]
            for i in range(n)
            if i != start
        ]
    )
    ...
    for i in range(y.shape[0]):
        ...
        cqm.add_constraint(
            quicksum([adjacent[start, k] * y[i, k] for k in range(n)]) == 1
        )
        cqm.add_constraint(
            quicksum([adjacent[end, k] * y[i, k] for k in range(n)]) == 1
        )
...
def avg_path_length(graph_list, W):
    n = len(W)
    avg_path = []
