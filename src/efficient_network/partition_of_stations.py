...
def divide_region(N, We, token):
    cqm = ConstrainedQuadraticModel()

    W = np.array(
        [We[j][i] if i > j else We[i][j] for i in range(N) for j in range(N)]
    ).reshape(N, N)
    W[range(N), range(N)] = 0  # diaganol element = 0

    x = np.array([Binary(i) for i in range(N)])

    objective = 0.5 * np.dot(1 - x, np.dot(W, (1 - x).transpose())) + 0.5 * np.dot(
        x, np.dot(W, x.transpose())
    )
...
def make_W(W, nodes):
    w = [
        0 if (i > j) or (i == j) else W[nodes[i]][nodes[j]]
