...
# Return a list of connected networks with minimum overall distances while satisfying the constraints
# Biggest stations are required to have at least 4 connections while smallest ones can have at most 2
def mini_overall_dist(n, We, Wn, token):
    ...
    graph_list = [
        [(i, j) for i in range(n) for j in range(i + 1, n) if value[f"x{i}{j}"] == 1]
...
# calculate the overall distances of a list of networks
def overall_length(graph_list, W):
    return np.array(
        [np.sum([W[edge[0], edge[1]] for edge in graph]) for graph in graph_list]
    )


# convert graph stored in the format [edge1, edge2,...] into nx.Graph object
def make_nx_graph(graph, W, nodes):
    g = nx.Graph()
    for i in nodes:
        g.add_node(i)
    for j in graph:
